...
def process_all_images_in_directory(input_dir, txt_dir, output_dir):
    ...
    for image_name in os.listdir(input_dir):
        if image_name.endswith(('.jpg', '.jpeg', '.png', '.tif')):  # Filter image files
...
# 处理多个子目录，每个子目录名字作为日期
region = 'Singapore'
base_input_dir = f"tank_file/{region}"  # Base directory containing subdirectories for each date
...
# 遍历每个子文件夹
for date in os.listdir(base_input_dir):
    date_dir = os.path.join(base_input_dir, date)
    if os.path.isdir(date_dir):
        ...
        info_dir = os.path.join(date_dir, 'more_info')  # Define info_dir within each date folder
        ...
        total_detection_boxes = 0  # 每个文件夹内的总检测框数
        ...
        with open(summary_path, 'w') as summary_file:
            ...
            summary_file.write(f"--- {date} Detection Summary ---\n")
            ...
            summary_file.write(f"Total boxes in {date}: {total_detection_boxes} ({percentage_overall:.2f}%)\n")

            # 转换成 YOLO 格式
            process_all_images_in_directory(input_dir, mid_dir, output_dir)

            print(f"All images in {date} have been converted to YOLO format!")

        # 保存 classes.txt
        with open(os.path.join(output_dir, 'classes.txt'), 'w') as f:
            f.write('tank\n')

print("所有子目录的图像标注处理完成！")
